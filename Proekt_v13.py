...
commands = ["❓ Поиск", "👋 Справка", "✅ Добавить документ", "🕹 Удалить ключ доступа", "📕 Тип", "📗 Название",
            ...
            "💼 Добавить/изменить ключ доступа", "✍️ Изменить документ"]
...
def get_button(message, lst):
    markup = types.ReplyKeyboardMarkup()
    for i in lst:
        markup.add(types.KeyboardButton(i))
    bot.send_message(message.chat.id, "Выберите нужный вид документа", reply_markup=markup)
...
def process_search_query(message, choice):
    ...
    if (choice == "📕 Тип"):
        ...
        resInd = searchByType(search_query)
        if (len(resInd) > 0):
            for i in range(len(resInd)):
                ...
                if check_user(message.from_user.id, res[0][0]):
                    ...
                    for i in res[0]:
                        ...
                        markup.add(InlineKeyboardButton(text=i[:10]))
                    bot.send_message(message.chat.id, "Выберите нужный вид документа", reply_markup=markup)
                else:
...
def process_document_number(message, document_type):
    ...
    bot.register_next_step_handler(message, process_entry_date, document_type, document_number)


def process_entry_date(message, document_type, document_name):
    entry_date = message.text
    bot.send_message(message.chat.id, "Введите дату входа (в формате ГГГГ-ММ-ДД 00:00:00):")
...
def process_exit_date(message, document_type, document_name, document_number):
    ...
    bot.register_next_step_handler(message, process_keyWords_date, document_type, document_name, document_number, exit_date)


def process_keyWords_date(message, document_type, document_name, document_number, entry_date):
    keyWords = message.text
    bot.send_message(message.chat.id, "Введите ключевые слова (разделите их запятыми):")
    bot.register_next_step_handler(message, save_document, document_type, document_name, document_number, entry_date, keyWords)
...
def save_document(message, document_type, document_name, document_number, entry_date, exit_date):
    ...
    try:
        ...
        cursor.execute('''INSERT INTO FirstOfAll (Type, Name, Number, ReleaseDate, StartDate, KeyWords)
                          VALUES (?, ?, ?, ?, ?, ?)''', (document_type, document_name, document_number, entry_date, exit_date, keywords))
        conn.commit()
